...
import { usePartial, usePipe } from '../src'

describe('use-fun-hooks', () => {
  ...
  test('should substitute arguments', () => {
    ...
    const Result = () => {
      const foo1 = usePartial(foo, 1)
      const foo2 = usePartial(foo, 2, true)
      const foo3 = usePartial(foo, 3, false)
      const foo4 = usePipe(foo1, foo2, foo3)

      return <div data-testid='result'>{foo4(false, 'foo')}</div>
    }
  ...
  test('should cast undefined to void', () => {
    ...
    const Result = () => {
      const fbb = usePartial(foo, bar, bar)
      const bb = usePartial(baz, bar)
      const foo3 = usePipe(fbb, bb)
  ...
  test('should keep returned function identity between renders unless arguments changed', () => {
    ...
    const Result = ({ arg }: { arg: number }) => {
      const foo1 = usePartial(foo, arg, true)
      const foo2 = usePartial(foo, 3, false)
      const foo3 = usePipe(foo, foo1, foo2)
